...
PDGLIB_API pdgerr_t pdg_getInterpMethod(char *interpType, long *pVal)
{
	try {
		...
		else if (!COMPARESTRING(interpType, "bspline")) *pVal = 4;
		else if (!COMPARESTRING(interpType, "kruger")) *pVal = 5;
      else if (!COMPARESTRING(interpType, "cspline") || !COMPARESTRING(interpType, "cubicspline")) *pVal = 6;
      else if (!COMPARESTRING(interpType, "hspline") || !COMPARESTRING(interpType, "hymanspline")) *pVal = 7;
...
PDGLIB_API pdgerr_t pdg_getBoundaryCondition(char *boundaryType, long *pVal)
{
	try {

		if (!COMPARESTRING(boundaryType, "notaknot")) *pVal = 0;
		else if (!COMPARESTRING(boundaryType, "firstderivative")) *pVal = 1;
		else if (!COMPARESTRING(boundaryType, "secondderivative"))  *pVal = 2;
		else if (!COMPARESTRING(boundaryType, "clamped")) *pVal = 3;
		else return pdg::Error(2,"Wrong boundary condition").getInfo();
