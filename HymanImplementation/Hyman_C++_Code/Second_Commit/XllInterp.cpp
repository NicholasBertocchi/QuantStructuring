 ...
 *  the user. (Example: "linear", bspline", "kruger", "cubicspline")
 *
 * \section Syntax
 *   xlInterp1D(InX, InY, OutX, InterpMethod, boundaryType, HymanFilter)
 *
 * \section Parameters
 ...
 *   \arg \a InterpMethod - Select the Interpolation Method ('linear', 'bspline', 'kruger', cubicspline); default value: 'linear'".
 *   \arg \a boundaryType - Optional: Select the boundary condition for Cubic Splines ('notaknot = 0', 'firstderivative = 1', 'secondderivative= 2', 'clamped' = 3).
 *   \arg \a Extrapolator - Optional: Select how to extrapolate before first and after last input point ('error = 0', 'flat = 1', 'linear = 2', 'extrap' = 3).
 *   \arg \a HymanFilter -  Optional: 'TRUE' if you want to apply Hyman monotonicity constrained filter; defualt value: 'FALSE'"
 *
 * \section Returns
 *    a vertical array of output Y coordinates
 *
 * \section Example
 *
 * \sa
...
LPXLOPER EXCEL_EXPORT xlInterp1D(
  ...
  XlfOper xlBoundaryType, //optional;
  XlfOper xlExtrapolator, //optional;
  XlfOper xlHymanFilter //optional; default value: 'FALSE'
){
  EXCEL_BEGIN;
  ...
  long boundaryType;
  boundaryType = xloper_cast<long>(xlBoundaryType);

  long Extrapolator;
  Extrapolator = xloper_cast<long>(xlExtrapolator);

  bool HymanFilter;
  HymanFilter = xloper_cast<bool>(xlHymanFilter);
  ...
  if ("linear" == method) {
    ...
    else if ("cspline" == method) {
    res = pdg_interpCubicSpline(
      &inX[0],
      &inY[0],
      inX.size(),
      &outX[0],
...
LPXLOPER EXCEL_EXPORT xlInterpLinear(
  ...
  XLOPER xlHymanFilter;
  xlHymanFilter.xltype = xltypeStr;
  xlHymanFilter.val.str = hyman;

  return xlInterp1D(xlInX, xlInY, xlOutX, &xlMethod, boundary, extrap, &xlHymanFilter);
...
/** \page xlInterpCubicSpline_page xlInterpCubicSpline
 *
 *  Returns an array containing the interpolation points relative
 *  to the outX coordinates. The Cubic Spline methodology
 *  is applied leaving the possibility to choose which boundary condition
 *  has to be use ('notaknot = 0', 'firstderivative = 1', 'secondderivative= 2', 'clamped' = 3").
 *  Furthermore it is possibile to apply the Hyman monotonicity constrained filter to the 
 *  Cubic Spline which implies that max and min are possible only on input points and not 
 *  between them avoiding the typical Cubic Splines' overshooting problem.
 *
 * \section Syntax
 *   xlInterpCubicSpline(InX, InY, OutX, BoundaryType, HymanFilter)
 *
 * \section Parameters
 *   \arg \a InX vertical array of input X coordinates.
 *   \arg \a InY vertical array of input Y coordinates.
 *   \arg \a OutX vertical array of output X coordinates.
 *   \arg \a boundaryType - Optional: Select Cubic Spline's boundary condition for Cubic Spline ('notaknot = 0', 'firstderivative = 1', 'secondderivative= 2', 'clamped' = 3);
 *   \arg \a Extrapolator - Optional: Select how to extrapolate before first and after last input point ('error = 0', 'flat = 1', 'linear = 2', 'extrap' = 3).
 *   \arg \a HymanFilter -  Optional: 'TRUE' if you want to apply Hyman monotonicity constrained filter; defualt value: 'FALSE'"
 *
 * \section Returns
...
LPXLOPER EXCEL_EXPORT xlInterpCubicSpline(
  XlfOper xlInX,
  XlfOper xlInY,
  XlfOper xlOutX,
  XlfOper xlBoundaryType,
  XlfOper xlExtrapolator,
  XlfOper xlHymanFilter
){
  EXCEL_BEGIN;

  char method[] = "\7cspline";

  XLOPER xlMethod;
  xlMethod.xltype = xltypeStr;
  xlMethod.val.str = method;

  return xlInterp1D(xlInX, xlInY, xlOutX, &xlMethod, xlBoundaryType, xlExtrapolator, xlHymanFilter);
